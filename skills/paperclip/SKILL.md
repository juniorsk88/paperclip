...
- **Do not create separate Git repos.** Use the repository/workspace Paperclip gives you as the canonical checkout. Do not run `git init`, clone the same repo elsewhere, create a separate worktree/repository, or create a task-specific branch unless the board/operator explicitly asks for that Git topology.
