...
function resolveKimiSkillsHome(config: Record<string, unknown>) {
  ...
  return path.join(home, ".kimi", "skills", "paperclip");
}
...
async function buildKimiSkillSnapshot(config: Record<string, unknown>): Promise<AdapterSkillSnapshot> {
  ...
  return buildPersistentSkillSnapshot({
    ...
    installed,
    skillsHome,
    locationLabel: "~/.kimi/skills/paperclip",
    installedDetail: "Installed in the Kimi Paperclip skills directory.",
    missingDetail: "Configured but not currently linked into the Kimi skills directory.",
...
export async function syncKimiSkills(
  ...
  for (const [name, installedEntry] of installed.entries()) {
    ...
    if (!available || !desiredSet.has(available.key)) {
